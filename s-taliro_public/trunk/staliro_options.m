...
% (C) 2015, Adel Dokhanchi, Arizona State University
    ...
    properties(Dependent)
        ...
        %
        %   The users can define their own optimization function. The current
        ...
        %
        %   Remark: If you are developing your own optimization function which
        %       needs parallelization, then you may consider using the parallelization
        %       functionality provided by the Compute_Robustness function.
        optimization_solver;
    ...
    properties
        ...
        %   it is completed. This option is useful for long runs:
        %   if you decide to kill the job halfway, you have something saved.
        %
        %   See save_intermediate_results_varname property for the name
        %   used.
        ...
        %
        %   If the model is a Simulink model with name 'mdl' and the option is set to
        %  'default', then the results will be saved in the filename
        %               'mdl_date_time_results.mat'.
        %
        %   Otherwise, the string stored in the property
        %               save_intermediate_results_varname
        %   will be used.
        ...
        % A string specifying the function for computing the temporal logic robustness
        % or any other custom cost function.
        %
        ...
        %   OR
        %      * a custom cost function defined by the user.
        %
        %   You can define your own cost function without considering the specification.
        ...
        %   accessible by Matlab. If the name of the function is "custom_cost", then the
        %   function needs to satisfy the following interface:
        %           rob = custom_cost( x, t, auxData )
        %   where
        %      * x is an m x n array containing the output trajectory where
        %            m is the number of samples, and
        %            n the dimensionality of the output vector
        %      * t is the vector with the time stamps of the output trajectory
        %      * auxData is a variable that can store any desired data and it is set
        ...
        %
        % See demo: staliro_demo_custom_cost_function.m
        taliro = 'dp_taliro';
        ...
        % Choose the number of runs against specific input if model under
        % test is stochastic.
		% 
		%   Default option : n_stochastic_runs = 2;
		%   
        %   Set the number of runs against specific input for a stochastic
        %   model-under-test and the mean is calculated using these
        %   n_stochastic_runs number of runs.
        n_stochastic_runs = 2;                                           
        ...
        % Collect detailed time statistics.
        % Enable this option in case you would like to collect statistics for how long the model runs
        % and how long the robustess computation takes. The option cannot
        % be used in
        %
        %   Default option : TimeStatsCollect = 0; % (or false; i.e., disabled)
        %
        % If the option is enbaled (1 or true), then the results and history structures returned by
        % staliro will contain the following additional information:
        %
        %       results.time
        %           where time is a structure with fields:
        %               totTime         : the total run time for each run
        %               simTime         : the total model simulation or SUT execution time
        %               ratioSimTime    : the ratio of simTime over total falsification time
        %               robTime         : the total robustness computation time
        %               ratioRobTime    : the ratio of robTime over total falsification time
        %               optimTime       : total time used by the optimizer
        %
        %       history.simTimes: the model simulation or SUT execution time for each test
        %       history.robTimes: the robustness computation time for each test
        TimeStatsCollect = 0;
