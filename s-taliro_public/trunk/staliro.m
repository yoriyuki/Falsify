...
%                   If the the option TimeStatsCollect in staliro_options 
...
%     In the current version of staliro this information is stored in
...
%% global declarations (read only)
% TODO: these are getting too many: Consolidate in a signle object?
...
%% global declarations (write); Avoid using inside a parfor loop
...
if iscell(model)
    ...
    % For backward compatibility of Blackbox option.
    % if user still using a Blackbox option in staliro options, then type cast model to Blackbox
...
results.RandState = set_staliro_seed(opt.seed);
...
% Statistics collected are on successful descents (not time stats)
global RUNSTATS;
...
if staliro_opt.parameterEstimation > 0
    ...
    assert(ischar(phi), ' S-TaLiRo : Currenlty, parameter estimation works only on a single STL/MTL specification.')
...
%% Prepare the collection of statistics
if opt.TimeStatsCollect 
    if ~isprop(opt.optim_params,'n_tests') || isempty(opt.optim_params.n_tests)
        n_tests = 0; % Inefficient: The arrays will be reallocated with every new time addition
    else
        n_tests = opt.optim_params.n_tests;
    end
    staliro_timeStats = STaliroTimeStats(n_tests); 
    staliro_timeStats.collectData(true);
end
RUNSTATS.resume_collecting();

%% Run tests
% GF: TODO: We should implemented a parfor here to split the runs over
% multiple cores for sequential optimization algorithms.
% In this case, if time statistics are collected, then we should create a
% new object for each worker and find the current worker inside Compute_Robustness.
for ii = 1:staliro_opt.runs
    
    if opt.TimeStatsCollect
        staliro_timeStats.reset;
    end
    RUNSTATS.new_run();
    ...
    if opt.TimeStatsCollect
        timeStats = struct('totTime',[],'simTime',[],'ratioSimTime',[],'robTime',[],'ratioRobTime',[],'optimTime',[]);
        timeStats.totTime = tmp_totTime;
        timeStats.simTime = staliro_timeStats.totalSimTime;
        timeStats.ratioSimTime = timeStats.simTime/tmp_totTime;
        timeStats.robTime = staliro_timeStats.totalRobTime;
        timeStats.ratioRobTime = timeStats.robTime/tmp_totTime;
        timeStats.optimTime = tmp_totTime-(timeStats.simTime+timeStats.robTime);
        results.run(ii).time = timeStats;
    else
        results.run(ii).time = tmp_totTime;
    end
    ...
    if staliro_opt.dispinfo>0
        disp(['  Running time of run ',num2str(ii),': ',num2str(tmp_totTime),' sec'])
    end
    ...
    if staliro_opt.save_intermediate_results
        ...
        if strcmp(opt.save_intermediate_results_varname,'default') && strcmp(staliro_InputModelType,'simulink')
            ...
            save([staliro_InputModel, '_', str_clock, '_results.mat'], 'intermediateStaliroResults');
        else        
