...
static Node	*tl_formula(int);
static Node	*tl_factor(int);
static Node	*tl_level(int,int);
...
static Node *bin_minimal(Node *ptr,int NNF)
{       
	Node *a, *b;
	...
	if (ptr)
	{
		...
		{
			case IMPLIES:
                ptr->lft->antecedent=1;
                if (NNF==1){
                    a = Not(ptr->lft);
        			return tl_nn(OR,a , ptr->rgt);
                }
			case EQUIV:
		        if (NNF==1){
...
static Node *tl_level(int nr,int NNF)
{	
	int i; Node *ptr = ZN;
	Interval LocInter;

	if (nr < 0)
		return tl_factor(NNF);

	ptr = tl_level(nr-1,NNF);
again:
	for (i = 0; i < 4; i++)
		...
		{	
			if (nr==0 && (i==0 || i==1))
				LocInter = TimeCon;
			tl_yychar = tl_yylex();
			ptr = tl_nn(prec[nr][i],ptr,tl_level(nr-1,NNF));
			if (nr==0 && (i==0 || i==1))
				ptr->time = LocInter;
			if(tl_simp_log) 
				ptr = bin_simpler(ptr);
			else 
				ptr = bin_minimal(ptr,NNF);
			goto again;
		}
...
tl_formula(int NNF)
{	tl_yychar = tl_yylex();
	return tl_level(1,NNF);	/* 2 precedence levels, 1 and 0 */	
}

Node *tl_parse(int NNF)
{  
   Node *n;
   n= tl_formula(NNF);
   if( NNF!=0 && NNF!=1 )
       mexErrMsgTxt("tl_parse Error: NNF value must be 0 or 1"); 
   if (tl_verbose)
	{	printf("formula: ");
		put_uform();
		printf("\n");
...
Node * tl_simpler(Node * phi){
    return bin_simpler(phi);
}

Node * tl_not(Node * phi){
    return Not(phi);
}
